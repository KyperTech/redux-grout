...
function callGrout(callInfoObj) {
  ...
  if (!(0, _lodash.isArray)(modelData)) {
    modelData = [modelData];
  }
  if (model) {
    grout = (0, _lodash.isObject)(modelData) || (0, _lodash.isString)(modelData) ? grout[model].apply(grout, modelData) : grout[model];
  }
  if (subModel) {
    grout = grout[subModel];
  }
  if (!(0, _lodash.isArray)(methodData)) {
