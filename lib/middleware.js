...
function callGrout(callInfoObj) {
  ...
  var callPiecesArray = [model, modelData, subModel, subModelData];
  (0, _lodash.each)(callPiecesArray, function (piece) {
    if (piece && typeof piece !== 'undefined') {
      promiseCall = (0, _lodash.isObject)(piece) ? promiseCall(piece) : promiseCall[piece];
    }
  });
  return promiseCall[method](methodData).then(function (response) {
    var camelizedJson = (0, _humps.camelizeKeys)(response);
    var endResult = Object.assign({}, (0, _normalizr.normalize)(camelizedJson, schema));
